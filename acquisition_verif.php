...
$viewData = [
	'date_acquisition' => $enCours ? sprintf(
		'<input name="date_acquisition" type="date" required value="%s">',
		htmlspecialchars(date('Y-m-d', strtotime($donnees['date_acquisition'])) ?? '', ENT_QUOTES, 'UTF-8')
	) : date('d/m/Y', strtotime($donnees['date_acquisition'])),
	'libelle' => htmlspecialchars($donnees['libelle']),
	'vendeur' => $enCours ? sprintf(
		'<input name="vendeur" type="text" required value="%s">',
		...
		<form method="post" enctype="multipart/form-data" id='form-controle'>
			...
			<table>
				<tbody>
					...
					<tr>
						<td width="30%"><label for="reference">Référence :</label></td>
						<td><?= $viewData['reference'] ?></td>
					</tr>
					<tr>
						<td><label for="vendeur">Vendeur :</label></td>
						<td><?= $viewData['vendeur'] ?></td>
					</tr>
					<tr>
