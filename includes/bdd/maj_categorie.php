...
*               - 'libelle' : string nom de la categorie (requis)
* @param int $id ID de la categorie à modifier
...
* ]
*/
function mise_a_jour_categorie(array $donnees, int $id, ?mysqli $connection = null): array {
    ...
    try {
        ...
        // 4. VERIFICATION DE L'UNICITE DU LIBELLE
        $libelle = trim($donnees['libelle']);
        $checkSql = "SELECT id FROM categorie WHERE libelle = ? AND id != ?";
        ...
        if ($checkStmt->num_rows > 0) {
            return [
                'success' => false,
                'affected_rows' => 0,
                'error' => 'Ce libellé existe déjà pour une autre catégorie'
            ];
        }
        
        // 5. PREPARATION DE LA REQUETE DE MISE A JOUR
        ...
        error_log("Erreur MySQL lors de la mise à jour catégorie $id: " . $e->getMessage());
        return [
            'success' => false,
            'affected_rows' => 0,
        ...
        error_log("Erreur lors de la mise à jour catégorie $id: " . $e->getMessage());
        return [
            'success' => false,
            'affected_rows' => 0,
            'error' => $e->getMessage()
