<?php
	...
	try {
		...
		if ($id === 0) {
			$stmt = $connection->prepare("SELECT MAX(id) AS max_id FROM lieu");
			$stmt->execute();
		...
		// Suppression
		if ($id > 1) {
			$stmt1 = $connection->prepare("DELETE FROM lieu WHERE id = ?");
		$stmt1->bind_param("i", $id);
		$stmt1->execute();
		}
		
		if ($connection->affected_rows > 0) {
			$avis = "Le lieu a été supprimé avec succès";
			$succes = true;
			
			// Réinitialisation auto_increment si table vide
			if (isset($_GET['edit'])) {
				$connection->query("ALTER TABLE lieu AUTO_INCREMENT = 1");
			}
		} elseif ($id < 2) {
			...
			$avis = "Aucun lieu trouvé à supprimer";
			$succes = false;
		}
		...
		$avis = "Une erreur technique est survenue lors de la suppression du lieu";
	}
