<?php
	...
	try {
		...
		if ($id === 0) {
			$stmt = $connection->prepare("SELECT MAX(id) AS max_id FROM utilisateur");
			$stmt->execute();
		...
		// Suppression
		$stmt1 = $connection->prepare("DELETE FROM utilisateur WHERE id = ?");
		...
		$avis = "Une erreur technique est survenue lors de la suppression de l'utilisateur";
	}
?>
