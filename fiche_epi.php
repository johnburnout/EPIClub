...
$viewData = [
	...
	'libelle' => $isLoggedIn ? sprintf(
		'<input name="libelle" type="text" required value="%s">',
		htmlspecialchars($donnees['libelle'] ?? '', ENT_QUOTES, 'UTF-8')
	) : htmlspecialchars($donnees['libelle'] ?? '', ENT_QUOTES, 'UTF-8'),
	'date_debut' => $isAdmin ? sprintf(
		'<input name="date_debut" type="date" required value="%s">',
		htmlspecialchars(date('Y-m-d', strtotime($donnees['date_debut'])) ?? '', ENT_QUOTES, 'UTF-8')
	) : htmlspecialchars(date('Y-m-d', strtotime($donnees['date_debut'])) ?? '', ENT_QUOTES, 'UTF-8'),
	'date_max' => $isAdmin ? sprintf(
		'<input name="date_max" type="date" required value="%s">',
		...
		<form enctype="multipart/form-data" method="post" action="fiche_epi.php" id='form-controle'>
			...
			<table>
				<tbody>
					...
					<tr>
						<td colspan="1" rowspan="2"><label for="remarque">Remarques :</label></td>
						<td>
					...
					<?php if ($isLoggedIn): ?>
						<tr>
							<td><textarea name="remarque" placeholder="Saisissez vos remarques..." rows="4" cols="40"></textarea></td>
						</tr>
						<tr>
							<td>
