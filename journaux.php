...
for ($i = 0; $i < 5; $i++) {
	...
	$annees_journaux[] = [
		...
		'existe' => file_exists(__DIR__.'/enregistrements/' . 'journal' . $annee . '.txt')
	];
...
for ($i = 0; $i < $total_controles; $i++) {
	$controles_journaux[] = [
	   'controle' => $donnees[$i]['date_verification'],
		...
		'existe' => file_exists(__DIR__.'/enregistrements/' . 'journalcontrole' . $donnees[$i]['id'] . '.txt')
	];
}
								...
								<tr>
									...
									<td>
										<?php if (!empty($annees_journaux[$i]['existe'])): ?>
											<a href="affichage_texte.php?csrf_token=<?= htmlspecialchars($csrf_token) ?>&url=<?= urlencode($annees_journaux[$i]['chemin']) ?>&retour=journaux.php&id=0">
											   <input type="button"
									...
									<td>
										<?php if (!empty($controles_journaux[$i]['existe'])): ?>
											<a href="affichage_texte.php?csrf_token=<?= htmlspecialchars($csrf_token) ?>&url=<?= urlencode($controles_journaux[$i]['chemin']) ?>&retour=journaux.php&id=0">
											   <input type="button"
